...
- **OWASP Security Response Headers:** Strict defense-in-depth headers applied across all endpoints (including static/cached HTTP 304 responses): `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'`, and `Referrer-Policy: strict-origin-when-cross-origin`.
- **Standardized OAuth 2.1 CORS Policy:** Strict cross-origin resource sharing configuration allowing preflight `OPTIONS` for browser-based clients on token endpoints with exposed `ETag` and `DPoP-Nonce` headers.
...
| **Edge Reverse Proxy Path-Based Isolation** | Configure internet-facing ALB / Nginx / Envoy to route standard OAuth paths (`/oauth2/**`, `/.well-known/**`, `/userinfo`) while returning fixed HTTP 403 on `/api/admin/**` and `/actuator/**`. Internal Next.js Client Manager connects via private VPC DNS (`http://spring-auth-server:9000`). | Blocks administrative registration endpoints at the public edge without placing network filtering overhead on JVM worker threads. | Requires configuring listener rules on edge load balancer. | **Deployment / Cloud Infrastructure Configuration** (Detailed in `docs/architecture/network_perimeter_and_proxy_routing.md`) |

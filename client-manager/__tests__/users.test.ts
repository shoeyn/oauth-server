...
describe('users api', () => {
  ...
  describe('createUser', () => {
    it('should resolve on success and hash password', async () => {
      ...
      await expect(createUser({ email: 'bob@example.com', password: 'password123' })).resolves.toBeUndefined();
    ...
    it('should throw on error', async () => {
      ...
      await expect(createUser({ email: 'bob@example.com' })).rejects.toThrow('Failed to create user: HTTP 500');
    });
  });

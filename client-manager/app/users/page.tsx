    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Manage platform users, passwords, and handle fraud interventions.
        </p>
      </div>
          ...
          >
            <div>
              ...
              <input
                ...
                name="email"
                required
                className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-white text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              />
            </div>
            <div>
              <label
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-indigo-500 shadow-lg shadow-indigo-600/20"
            >
              Create User
            </button>
          </form>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl flex flex-col">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-800">
            <thead className="bg-slate-800/50">
              <tr>
                <th className="py-4 pl-6 pr-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">
                  ID
                </th>
            ...
            <tbody className="divide-y divide-slate-800 bg-slate-900">
              {data.users.map((u) => (
                <tr key={u.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="whitespace-nowrap py-4 pl-6 pr-3 text-xs font-mono text-slate-500">
                    {u.id.split("-")[0]}...
                  </td>
                    ...
                    {editEmail === u.email ? (
                      <form id={`edit-${u.id}`} action={editUserAction} className="flex gap-2">
                        ...
                        <input
                          ...
                          className="px-2 py-1 rounded bg-slate-950 border border-slate-800 text-white text-xs"
                        />
                        <input
                          type="password"
                          name="password"
                          ...
                          className="px-2 py-1 rounded bg-slate-950 border border-slate-800 text-white text-xs w-32"
                        />
                      </form>
                    ) : (
                      u.email
                    ...
                    <div className="flex justify-end gap-3 items-center">
                      {editEmail === u.email ? (
                        <div className="flex gap-2">
                          ...
                          <Link href="/users" className="text-slate-400 hover:text-slate-300">
                            Cancel
                          </Link>
                        </div>
                      ) : (
                        <Link
                          href={`/users?edit=${encodeURIComponent(u.email)}`}
        ...
        <div className="px-6 py-4 border-t border-slate-800 bg-slate-800/30 flex items-center justify-between">
          <div className="text-sm text-slate-400">
            ...
            <span className="font-semibold text-white">{Math.max(1, data.totalPages)}</span> (

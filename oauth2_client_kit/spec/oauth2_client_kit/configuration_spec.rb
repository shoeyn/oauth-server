...
RSpec.describe OAuth2ClientKit::Configuration do
  ...
  describe '#initialize' do
    ...
    it 'falls back to OAUTH2_CLIENT_ID when CLIENT_ID is unset' do
      ...
      allow(ENV).to receive(:[]).with("OAUTH2_CLIENT_ID").and_return("fallback_client")
    ...
    it 'uses AUTH_SERVER_URL_INTERNAL for the internal issuer when present' do
      allow(ENV).to receive(:[]).and_call_original
      allow(ENV).to receive(:[]).with("AUTH_SERVER_URL_INTERNAL").and_return("http://internal:9000")
    ...
    it 'falls back the internal issuer to the public issuer when the internal env is blank' do
      allow(ENV).to receive(:[]).and_call_original
      allow(ENV).to receive(:[]).with("AUTH_SERVER_URL_INTERNAL").and_return("")
      allow(ENV).to receive(:fetch).and_call_original
    ...
    it 'sets a 30 day token cache ttl' do
      expect(subject.token_cache_ttl).to eq(30 * 24 * 60 * 60)
    end

    it 'sets default post-login and post-logout paths' do
      expect(subject.after_login_path).to eq("/profile")
      expect(subject.after_logout_path).to eq("/")
    end
  end
  ...
  describe '#resolved_private_key_pem' do
    it 'returns the PEM of an OpenSSL::PKey::RSA object' do
      ...
      subject.private_key = key
      expect(subject.resolved_private_key_pem).to eq(key.to_pem)
    end

    it 'returns a string private_key unchanged (via to_s)' do
      subject.private_key = "-----BEGIN PRIVATE KEY-----\nabc\n-----END PRIVATE KEY-----"
      expect(subject.resolved_private_key_pem).to eq("-----BEGIN PRIVATE KEY-----\nabc\n-----END PRIVATE KEY-----")
    end
    ...
    it 'reads the key from private_key_path when the file exists' do
      ...
      subject.private_key_path = file.path
      expect(subject.resolved_private_key_pem).to eq("PEM-FROM-FILE")
    ensure
      file.close!
    end
    ...
    it 'returns nil when the private_key_path does not exist' do
      subject.private_key = nil
      subject.private_key_path = "/nonexistent/path/key.pem"
      expect(subject.resolved_private_key_pem).to be_nil
    end

...
RSpec.describe OAuth2ClientKit::Client do
  ...
  before do
    ...
    described_class.class_variable_set(:@@cached_jwks, {})
  end

  describe '#initialize' do
    it 'sets URLs correctly' do
      expect(subject.public_issuer_url).to eq(public_issuer_url)
      expect(subject.internal_issuer_url).to eq(internal_issuer_url)
      expect(subject.par_url).to eq("#{internal_issuer_url}/oauth2/par")
    end
    
    it 'accepts ENV["AUTH_SERVER_URL_INTERNAL"] for internal issuer' do
      allow(ENV).to receive(:[]).with("AUTH_SERVER_URL_INTERNAL").and_return("http://env.example.com")
  ...
  describe '#build_client_assertion' do
    it 'creates a valid signed JWT' do
      jwt = subject.build_client_assertion
  ...
  describe '#push_authorization_request' do
    it 'posts to par_url and returns request_uri' do
      stub_request(:post, subject.par_url).to_return(
        status: 201,
        body: { "request_uri" => "urn:foo" }.to_json
      )
      
      uri = subject.push_authorization_request({ scope: "openid" })
    ...
    it 'raises error on failure' do
      ...
      expect {
        subject.push_authorization_request({})
      }.to raise_error(/PAR Request Failed: invalid_request/)
    end
    ...
    it 'raises error if no request_uri in response' do
      ...
      expect {
        subject.push_authorization_request({})
      }.to raise_error(/No request_uri returned/)
    end
  end
  ...
  describe '#build_dpop_proof' do
    ...
    it 'builds a valid DPoP proof' do
      proof = subject.build_dpop_proof("POST", "http://example.com/token", nil, dpop_key)
    ...
    it 'includes ath claim if access_token provided' do
      proof = subject.build_dpop_proof("POST", "http://example.com/token", "token123", dpop_key)
      decoded = JWT.decode(proof, dpop_key, true, { algorithm: 'ES256' })
    ...
    it 'raises error if no key' do
      expect {
        subject.build_dpop_proof("POST", "url")
      }.to raise_error(/Missing DPoP private key/)
    end
    
    it 'supports RSA key' do
      rsa_key = described_class.generate_dpop_key(:rsa)
      proof = subject.build_dpop_proof("POST", "http://example.com/token", nil, rsa_key)
  ...
  describe '#parse_dpop_key' do
    it 'parses string keys' do
      ...
      parsed = subject.parse_dpop_key(ec_key.to_pem)
      expect(parsed).to be_a(OpenSSL::PKey::EC)
      
      rsa_key = described_class.generate_dpop_key(:rsa)
      parsed2 = subject.parse_dpop_key(rsa_key.to_pem)
  ...
  describe '#exchange_code' do
    ...
    it 'exchanges code successfully' do
      token = subject.exchange_code('code123', 'verifier123', 'http://cb')
    ...
    it 'adds DPoP header if dpop_key provided' do
      ...
      subject.exchange_code('code123', 'verifier123', 'http://cb', dpop_key)
    ...
    it 'handles DPoP nonce retry' do
      ...
      token = subject.exchange_code('code123', 'verifier123', 'http://cb', dpop_key)
    ...
    it 'raises error if nonce retry fails without dpop_key' do
      ...
      expect { subject.exchange_code('code123', 'verifier', 'http') }.to raise_error(OAuth2::Error)
    end
  end
  ...
  describe '#refresh_access_token' do
    ...
    it 'refreshes token successfully' do
      ...
      token = subject.refresh_access_token('refresh_token')
    ...
    it 'handles DPoP nonce retry' do
      ...
      token = subject.refresh_access_token('code123', dpop_key)
      expect(token.token).to eq('acc_token2')
    end
    
    it 'raises error if refresh fails without nonce' do
      stub_request(:post, token_url).to_return(status: 400)
      expect { subject.refresh_access_token('code123') }.to raise_error(OAuth2::Error)
    end
  end
  ...
  describe '#introspect_token' do
    ...
    it 'returns false for blank token' do
      expect(subject.introspect_token('')).to eq({ "active" => false })
    end
    ...
    it 'returns active status' do
      ...
      expect(subject.introspect_token('token123')['active']).to be true
    end
    
    it 'returns inactive on 400' do
      stub_request(:post, introspect_url).to_return(status: 400)
      expect(subject.introspect_token('token123')['active']).to be false
    end
  end

  describe '#verify_active!' do
    it 'returns claims if active' do
      allow(subject).to receive(:introspect_token).and_return({ "active" => true, "sub" => "user1" })
      res = subject.verify_active!('token123')
      expect(res[:active]).to be true
      expect(res[:sub]).to eq('user1')
    end
    
    it 'returns inactive reason if not active' do
      allow(subject).to receive(:introspect_token).and_return({ "active" => false })
      res = subject.verify_active!('token123')
  ...
  describe '#revoke_token' do
    ...
    it 'returns false for blank token' do
      expect(subject.revoke_token('')).to be false
    end
    
    it 'returns true on success' do
      stub_request(:post, revoke_url).to_return(status: 200)
      expect(subject.revoke_token('token123')).to be true
    end
    
    it 'returns false on failure' do
      stub_request(:post, revoke_url).to_return(status: 400)
      expect(subject.revoke_token('token123')).to be false
    end
  end

  describe '#end_session_url' do
    it 'constructs url' do
      url = subject.end_session_url('hint', 'http://cb')
  ...
  describe '#fetch_userinfo' do
    ...
    it 'returns empty hash for blank token' do
      expect(subject.fetch_userinfo('')).to eq({})
    end
    ...
    it 'fetches userinfo successfully' do
      ...
      expect(subject.fetch_userinfo('token123')).to eq({ "sub" => "user1" })
    end
    ...
    it 'uses DPoP if key provided' do
      ...
      subject.fetch_userinfo('token123', dpop_key)
      expect(WebMock).to have_requested(:get, userinfo_url).with { |req|
    ...
    it 'returns empty on failure' do
      ...
      expect(subject.fetch_userinfo('token123')).to eq({})
    end
  end
  ...
  describe 'JWT verifications' do
    ...
    describe '#fetch_jwks' do
      ...
      it 'handles fetch errors' do
        ...
        expect(subject.fetch_jwks).to be_nil
      end
      ...
      it 'returns nil if jwk_set_contains_kid? matches but fetch fails' do
        ...
        expect(subject.jwk_set_contains_kid?([jwk.export.merge('kid' => '123')], '123')).to be true
      end
    end
    ...
    describe '#decode_and_verify_id_token' do
      it 'verifies valid token' do
        ...
        expect(subject.decode_and_verify_id_token(token, nil)).to eq(payload.transform_keys(&:to_s))
      end
      
      it 'returns empty hash if blank token' do
        expect(subject.decode_and_verify_id_token(nil, nil)).to eq({})
      end
      ...
      it 'validates nonce' do
        ...
        token = sign_jwt(payload)
        expect {
          subject.decode_and_verify_id_token(token, 'wrong')
        }.to raise_error(/nonce.*does not match/)
      end
      ...
      it 'validates at_hash' do
        ...
        token = sign_jwt(payload)
        expect(subject.decode_and_verify_id_token(token, nil, at)).to be_present
      ...
      it 'validates c_hash' do
        ...
        token = sign_jwt(payload)
        expect(subject.decode_and_verify_id_token(token, nil, nil, code)).to be_present
    ...
    describe '#decode_and_verify_jarm_response' do
      it 'verifies valid jarm' do
        ...
        token = sign_jwt(payload)
        expect(subject.decode_and_verify_jarm_response(token, 's123')).to be_present
      end
      
      it 'raises on missing token' do
        expect { subject.decode_and_verify_jarm_response(nil) }.to raise_error(/Missing JARM/)
      end
      
      it 'raises on state mismatch' do
        payload = { iss: public_issuer_url, aud: client_id, exp: Time.now.to_i + 3600, state: 's123' }
        token = sign_jwt(payload)
        expect { subject.decode_and_verify_jarm_response(token, 'wrong') }.to raise_error(/state parameter mismatch/)
      end
    end
    ...
    describe '#verify_logout_token' do
      ...
      it 'raises if nonce is present' do
        ...
        token = sign_jwt(payload)
        expect { subject.verify_logout_token(token) }.to raise_error(/MUST NOT contain a nonce/)
      end
    end
    ...
    describe 'JWT signature verification details' do
      it 'rejects invalid algorithm' do
        ...
        expect { subject.decode_and_verify_id_token(token, nil) }.to raise_error(/must use 'RS256'/)
      end
      
      it 'rejects invalid issuer' do
        token = sign_jwt({ iss: 'wrong', aud: client_id, exp: Time.now.to_i + 3600 })
        expect { subject.decode_and_verify_id_token(token, nil) }.to raise_error(JWT::InvalidIssuerError)
      end
      
      it 'rejects invalid audience' do
        token = sign_jwt({ iss: public_issuer_url, aud: 'wrong', exp: Time.now.to_i + 3600 })
        expect { subject.decode_and_verify_id_token(token, nil) }.to raise_error(JWT::InvalidAudError)
      end
      
      it 'rejects expired token' do
        token = sign_jwt({ iss: public_issuer_url, aud: client_id, exp: Time.now.to_i - 3600 })
        expect { subject.decode_and_verify_id_token(token, nil) }.to raise_error(JWT::ExpiredSignature)
      end
      
      it 'rejects future issued token' do
        token = sign_jwt({ iss: public_issuer_url, aud: client_id, exp: Time.now.to_i + 3600, iat: Time.now.to_i + 3600 })
        expect { subject.decode_and_verify_id_token(token, nil) }.to raise_error(/issued in the future/)
      end
      ...
      it 'raises on missing JWKS' do
        ...
        token = sign_jwt({ iss: public_issuer_url, aud: client_id, exp: Time.now.to_i + 3600 })
        expect { subject.decode_and_verify_id_token(token, nil) }.to raise_error(/Unable to fetch JWKS/)
      end
    end
  end
  ...
  describe '#with_retries' do
    it 'retries and then raises' do
      count = 0
      expect {
        subject.with_retries(max_retries: 2, base_delay: 0.01) do
          count += 1

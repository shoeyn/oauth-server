...
| **Default Client** | `demo-client` | Asymmetric EC Private Key | `demo-client/keys/client_private_key.pem` | Main test client (`private_key_jwt`, DPoP, PAR) |
| **KMS Signing Key** | `kms-auth-server-key-1` | AWS KMS / LocalStack (ECC_NIST_P256) | Alias: `alias/oauth2-signing-key` | Token signing & JWKS public key verification |

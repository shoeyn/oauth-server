...
public class SharedRedisSessionFilterTest {
    ...
    void testConstructors() {
        ...
        SharedRedisSessionFilter f3 = new SharedRedisSessionFilter(redisTemplate, JsonMapper.shared(), "prefix");
        assertNotNull(f1);
        assertNotNull(f2);
        assertNotNull(f3);
    }
    ...
    void doFilterInternal_WithNullRoles_Succeeds() throws Exception {
        ...
        when(request.getCookies()).thenReturn(new Cookie[]{new Cookie(SharedRedisSessionFilter.COOKIE_NAME, sessionId)});
    ...
    void doFilterInternal_malformedSessionIdLength() throws Exception {
        ...
        when(request.getCookies()).thenReturn(new Cookie[]{new Cookie("SHARED_SESSION_ID", badSessionId)});
        when(request.getSession(false)).thenReturn(null);

        filter.doFilterInternal(request, response, filterChain);

        verify(response).addCookie(argThat(c -> c.getName().equals("SHARED_SESSION_ID") && c.getMaxAge() == 0 && "/".equals(c.getPath()) && c.isHttpOnly()));
        verify(filterChain).doFilter(request, response);
    }
    ...
    void doFilterInternal_malformedSessionIdNotUuid() throws Exception {
        ...
        verify(response).addCookie(argThat(c -> c.getName().equals("SHARED_SESSION_ID") && c.getMaxAge() == 0));
        verify(filterChain).doFilter(request, response);
    }
    ...
    void doFilterInternal_validSessionId_noRedisSession() throws Exception {
        ...
        when(request.getCookies()).thenReturn(new Cookie[]{new Cookie("SHARED_SESSION_ID", sessionId.toString())});
        when(redisTemplate.opsForValue()).thenReturn(valueOperations);
    ...
    void doFilterInternal_validSessionId_successWithAllFields() throws Exception {
        ...
        when(request.getCookies()).thenReturn(new Cookie[]{new Cookie("SHARED_SESSION_ID", sessionId.toString())});
        when(redisTemplate.opsForValue()).thenReturn(valueOperations);
        String json = "{\"username\":\"testuser\",\"email\":\"test@example.com\",\"name\":\"Test User\",\"roles\":[\"USER\",\"ROLE_ADMIN\"],\"authenticated_at\":\"2023-10-01T12:00:00Z\"}";

...
class DPoPNonceFilterTest {
  ...
  @InjectMocks private DPoPNonceFilter filter;
  ...
  void shouldNotFilter_ReturnsTrue_ForNonTokenPath() throws Exception {
    ...
    Method method =
        DPoPNonceFilter.class.getDeclaredMethod("shouldNotFilter", HttpServletRequest.class);
  ...
  @Test
  void doFilterInternal_PassesThrough_WhenBasicAuthHeaderPresent()
  ...
  @Test
  void doFilterInternal_ChecksDPoP_WhenBearerAuthHeaderPresent()
      throws ServletException, IOException {
    when(request.getRequestURI()).thenReturn("/oauth2/token");
  ...
  @Test
  void doFilterInternal_PassesThrough_WhenBasicAuthHeaderPresentDifferentCase()
      throws ServletException, IOException {
    when(request.getRequestURI()).thenReturn("/oauth2/token");
    when(request.getHeader(HttpHeaders.AUTHORIZATION)).thenReturn("basic dXNlcjpwYXNz");
  ...
  @Test
  void doFilterInternal_PassesThrough_WhenClientSecretParameterPresent()
      throws ServletException, IOException {
    when(request.getRequestURI()).thenReturn("/oauth2/token");
  ...
  void doFilterInternal_Rejects_WhenDPoPMissingNonceClaim() throws Exception {
    ...
    SignedJWT signedJWT =
        new SignedJWT(new JWSHeader(JWSAlgorithm.HS256), new JWTClaimsSet.Builder().build());
  ...
  void doFilterInternal_Rejects_WhenNonceInvalidOrExpired() throws Exception {
    ...
    SignedJWT signedJWT =
        new SignedJWT(
            ...
            new JWTClaimsSet.Builder().claim("nonce", nonce).build());
    signedJWT.sign(signer);
    when(request.getHeader("DPoP")).thenReturn(signedJWT.serialize());

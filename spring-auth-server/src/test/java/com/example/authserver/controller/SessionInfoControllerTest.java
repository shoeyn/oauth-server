...
class SessionInfoControllerTest {
  ...
  @Mock private OidcBackChannelLogoutService oidcBackChannelLogoutService;

  @Mock private RegisteredClientRepository registeredClientRepository;

  @InjectMocks private SessionInfoController controller;
  ...
  void revokeSession_Unauthorized_WhenApiKeyMissing() throws Exception {
    mockMvc
        .perform(post("/api/admin/revoke-session"))
  ...
  void revokeSession_Unauthorized_WhenApiKeyWrong() throws Exception {
    mockMvc
        .perform(post("/api/admin/revoke-session").header("X-Admin-Api-Key", "wrong"))
        .andExpect(status().isUnauthorized());
  }
  ...
  void revokeSession_ResolvesTokenAndDispatchesBackChannelLogout() throws Exception {
    ...
    verify(oidcBackChannelLogoutService)
        .dispatchLogout(isNull(), eq("erin-client"), eq("erin"), eq("sid-from-token"));
  }
  ...
  void revokeSession_FallsBackToRefreshTokenLookup() throws Exception {
    ...
    when(authorizationService.findByToken(eq(token), eq(OAuth2TokenType.ACCESS_TOKEN)))
        .thenReturn(null);
    when(authorizationService.findByToken(eq(token), eq(OAuth2TokenType.REFRESH_TOKEN)))
        .thenReturn(authorization);
  ...
  void me_UsesEmptyDetails_WhenAuthenticationDetailsNull() throws Exception {
    ...
    TestingAuthenticationToken auth =
        new TestingAuthenticationToken("noDetails", "n/a", "ROLE_USER");
  ...
  void revokeSession_KeepsProvidedUsernameAndSession_OverToken() throws Exception {
    ...
    when(revocationService.revokeUserGlobally(
            eq("explicit-user"), eq(authorization), eq("explicit-sid")))
        .thenReturn(result(1, true));
  ...
  void me_IncludesDetails_WhenAuthenticationDetailsPresent() throws Exception {
    TestingAuthenticationToken auth =
        new TestingAuthenticationToken("withDetails", "n/a", "ROLE_USER");
  ...
  void revokeSession_BlankUsernameParam_ResolvesUsernameFromToken() throws Exception {
    ...
    when(authorizationService.findByToken(eq(token), eq(OAuth2TokenType.ACCESS_TOKEN)))
        .thenReturn(authorization);
    when(revocationService.revokeUserGlobally(
            eq("resolved-from-token"), eq(authorization), isNull()))
        .thenReturn(result(1, false));

...
public class ClientAdminControllerTest {
  ...
  @Mock private PostgresRegisteredClientRepository clientRepository;

  @Mock private StringRedisTemplate redisTemplate;

  @InjectMocks private ClientAdminController controller;
  ...
  private RegisteredClient createClient(String clientId, String clientName, Boolean requirePar) {
    ClientSettings.Builder settingsBuilder =
        ClientSettings.builder().requireProofKey(true).requireAuthorizationConsent(true);
    if (requirePar != null) {
  ...
  void listClients_Unauthorized() throws Exception {
    ...
    mockMvc
        .perform(get("/api/admin/clients").header("X-Admin-Api-Key", ""))
        .andExpect(status().isUnauthorized());

    mockMvc
        .perform(get("/api/admin/clients").header("X-Admin-Api-Key", "wrong-key"))
        .andExpect(status().isUnauthorized());
  }
  ...
  void listClients_Success() throws Exception {
    ...
    when(client2.getClientAuthenticationMethods())
        .thenReturn(java.util.Set.of(ClientAuthenticationMethod.CLIENT_SECRET_BASIC));
    when(client2.getAuthorizationGrantTypes())
        .thenReturn(java.util.Set.of(AuthorizationGrantType.AUTHORIZATION_CODE));
    when(client2.getRedirectUris()).thenReturn(java.util.Set.of("http://localhost/callback"));
    when(client2.getPostLogoutRedirectUris())
        .thenReturn(java.util.Set.of("http://localhost/logout"));
    when(client2.getScopes()).thenReturn(java.util.Set.of("openid"));
    when(client2.getClientSettings())
        .thenReturn(
            ClientSettings.builder()
                .requireProofKey(true)
                .requireAuthorizationConsent(true)
                .build());
    when(client2.getTokenSettings())
        .thenReturn(
            TokenSettings.builder()
                .accessTokenTimeToLive(Duration.ofMinutes(15))
                .refreshTokenTimeToLive(Duration.ofDays(30))
    ...
    mockMvc
        .perform(get("/api/admin/clients").header("X-Admin-Api-Key", "secret-admin-key"))
        ...
        .andExpect(
            jsonPath("$[1].requirePushedAuthorizationRequests").value(true)) // defaults to true
  ...
  void getClient_NotFound() throws Exception {
    ...
    mockMvc
        .perform(get("/api/admin/clients/unknown").header("X-Admin-Api-Key", "secret-admin-key"))
  ...
  void getClient_Success() throws Exception {
    ...
    mockMvc
        .perform(get("/api/admin/clients/client1").header("X-Admin-Api-Key", "secret-admin-key"))
        .andExpect(status().isOk())
  ...
  void getClient_Unauthorized() throws Exception {
    mockMvc.perform(get("/api/admin/clients/client1")).andExpect(status().isUnauthorized());
  }

  @Test
  void saveClient_Unauthorized() throws Exception {
    mockMvc
        .perform(post("/api/admin/clients").contentType(MediaType.APPLICATION_JSON).content("{}"))
        .andExpect(status().isUnauthorized());
  }
  ...
  void saveClient_Success() throws Exception {
    ClientConfigDto dto =
        new ClientConfigDto(
            "new-client", null, null, null, null, null, null, null, null, null, null, null, null);
  ...
  void deleteClient_Unauthorized() throws Exception {
    mockMvc.perform(delete("/api/admin/clients/client1")).andExpect(status().isUnauthorized());
  }

  @Test
  void deleteClient_Success() throws Exception {
    mockMvc
        .perform(delete("/api/admin/clients/client1").header("X-Admin-Api-Key", "secret-admin-key"))
        .andExpect(status().isOk())

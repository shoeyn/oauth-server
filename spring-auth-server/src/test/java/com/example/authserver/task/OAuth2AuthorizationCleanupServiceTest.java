...
class OAuth2AuthorizationCleanupServiceTest {
    ...
    @InjectMocks
    private OAuth2AuthorizationCleanupService service;
    ...
    void pruneExpiredAuthorizations_ReturnsDeletedCount_OnSuccess() {
        when(jdbcTemplate.update(anyString(), any(Timestamp.class), any(Timestamp.class), any(Timestamp.class)))
                .thenReturn(7);

        int deleted = service.pruneExpiredAuthorizations(30);

        assertEquals(7, deleted);
        verify(jdbcTemplate).update(anyString(), any(Timestamp.class), any(Timestamp.class), any(Timestamp.class));
    }

...
/**
 * Scheduled background service to automatically prune expired OAuth 2.1 authorizations from
 * PostgreSQL (oauth2_authorization), preventing table bloat and maintaining B-tree index
 * performance.
 */
@Slf4j
@Service
...
public class OAuth2AuthorizationCleanupService {
  ...
  private static final String DELETE_EXPIRED_SQL =
      """
        ...
        WHERE (refresh_token_expires_at IS NOT NULL AND refresh_token_expires_at < ?)
           OR (refresh_token_expires_at IS NULL AND access_token_expires_at IS NOT NULL
               AND access_token_expires_at < ?)
           OR (refresh_token_expires_at IS NULL AND access_token_expires_at IS NULL
               AND authorization_code_expires_at IS NOT NULL AND authorization_code_expires_at < ?)
        """;
  ...
  public int pruneExpiredAuthorizations(int daysOld) {
    ...
    log.info(
        "Starting automated pruning of expired OAuth2 authorizations older than {} days (threshold: {})",
        daysOld,
        threshold);

    try {
      int rowsDeleted =
          jdbcTemplate.update(DELETE_EXPIRED_SQL, thresholdTs, thresholdTs, thresholdTs);
      log.info(
          "Completed automated OAuth2 authorization pruning: removed {} expired authorization record(s)",
          rowsDeleted);
      return rowsDeleted;
    } catch (Exception e) {

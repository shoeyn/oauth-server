...
/**
 * Enforces RFC 9449 Section 8 Server-Provided Nonces. Protects DPoP proofs against replay attacks
 * within the clock-skew window.
 *
 * <p>Flow: 1. For requests to /oauth2/token, if client authentication is valid (e.g.
 * client_assertion present), extracts the 'nonce' claim from the DPoP proof. 2. Checks if the nonce
 * exists in Redis (key: "dpop_nonce:" + nonce). 3. If missing, invalid, or expired: - Generates a
 * fresh server nonce and stores it in Redis with a 60-second TTL. - Sets the HTTP response header
 * "DPoP-Nonce: <new-nonce>". - Responds with HTTP 400 Bad Request and error "use_dpop_nonce" per
 * RFC 9449 Section 8. 4. If valid, consumes (deletes) the single-use nonce from Redis and permits
...
public class DPoPNonceFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    // If client attempted basic auth or client_secret_post, let downstream filters reject with
    // invalid_client
    ...
    if (!validNonce) {
      ...
      response
          .getWriter()
          .write(
              "{\"error\":\"use_dpop_nonce\",\"error_description\":\"Authorization server requires a valid DPoP-Nonce header\"}");
      return;
    }

    // On successful validation, supply a fresh nonce in the response header for future pipelined
    // requests

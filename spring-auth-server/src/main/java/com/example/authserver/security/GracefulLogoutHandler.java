...
/**
 * Handles OIDC logout failures gracefully when the id_token_hint is expired but the JWT signature
 * is still cryptographically valid.
 *
 * <p>If the token was genuinely issued by this server (signature verifies) and the
 * post_logout_redirect_uri is registered for the client, we treat it as a valid logout rather than
...
public class GracefulLogoutHandler {
  ...
  /**
   * Attempts a graceful logout fallback. Returns true if the fallback succeeded and the response
   * has been committed (redirect sent). Returns false if the fallback could not be applied and the
   * caller should handle the error normally.
   */
  public boolean handleGracefully(HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    ...
    try {
      ...
      if (state != null && !state.isBlank()) {
        targetRedirectUri =
            UriComponentsBuilder.fromUriString(normalisedRedirectUri)
      ...
      log.info(
          "Graceful logout fallback: id_token_hint expired but signature verified. Redirecting to {}",
          targetRedirectUri);
  ...
  /**
   * Evicts the SHARED_SESSION_ID cookie from Redis and clears it from the browser. This is a shared
   * utility used by both the success and error logout paths.
   */
  ...
  /**
   * Normalises a URI for safe comparison — strips fragments, normalises scheme/host case, and
   * removes default ports to prevent open redirect via encoding tricks.
   */

...
/**
 * OpenID Connect Back-Channel Logout 1.0 service. Asynchronously generates and delivers signed
 * logout_token JWTs to registered client back-channel logout endpoints, enabling direct
 * server-to-server session invalidation.
 */
@Slf4j
@Service
public class OidcBackChannelLogoutService {
  ...
  public void dispatchLogout(String clientLogoutUri, String clientId, String sub, String sid) {
    if (clientId == null || clientId.isBlank()) {
      log.warn(
          "Skipping back-channel logout: clientId is required but was null/blank (sub={})", sub);
      return;
    }

    String targetUri =
        (clientLogoutUri != null && !clientLogoutUri.isBlank())
            ? clientLogoutUri
    ...
    try {
      ...
      JwtClaimsSet.Builder claimsBuilder =
          JwtClaimsSet.builder()
              ...
              // MUST contain the "events" claim with
      ...
      java.util.concurrent.CompletableFuture.runAsync(
          () -> {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++) {
              try {
                log.info(
                    "Dispatching OIDC Back-Channel Logout to {} (sub={}, sid={}, attempt={}/{})",
                    targetUri,
                    sub,
                    sid,
                    attempt,
                ...
                if (attempt < maxRetries) {
                  ...
                  log.warn(
                      "OIDC Back-Channel Logout attempt {} failed for {}: {}. Retrying in {}ms...",
                      attempt,
                      targetUri,
                      e.getMessage(),
                      backoffMs);
                  ...
                  log.warn(
                      "Failed to deliver OIDC Back-Channel Logout to {} after {} attempts: {}",
                      targetUri,
                      maxRetries,
                      e.getMessage());
                }

...
/**
 * Service orchestrating user session and authorization token revocation across PostgreSQL and
 * Redis.
 */
@Slf4j
@Service
...
public class UserSessionRevocationService {
  ...
  /**
   * Revokes all authorizations for the specified user or authorization, and terminates active Redis
   * SSO sessions.
   */
  public RevocationResult revokeUserGlobally(
      String resolvedUsername, OAuth2Authorization singleAuthorization, String resolvedSessionId) {
    ...
    if (resolvedUsername != null && !resolvedUsername.isBlank()) {
      purgedAuthorizations =
          jdbcTemplate.update(
              "DELETE FROM oauth2_authorization WHERE principal_name = ?", resolvedUsername);
      log.info(
          "Force signed out user '{}' everywhere: purged {} authorizations from PostgreSQL",
          resolvedUsername,
          purgedAuthorizations);
      ...
      try (Cursor<String> cursor =
        ...
        while (cursor.hasNext()) {
          ...
          if (sessionJson != null && sessionJson.contains(usernameFragment)) {
            ...
            sessionEvicted = true;
            log.info(
                "Globally evicted active SSO Redis session {} for user {}", sKey, resolvedUsername);
          }
        }
      } catch (Exception ex) {

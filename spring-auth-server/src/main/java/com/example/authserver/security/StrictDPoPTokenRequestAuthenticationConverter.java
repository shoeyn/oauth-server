...
/**
 * Enforces RFC 9449 Demonstrating Proof-of-Possession (DPoP) on token endpoint requests. Rejects
 * token grant requests that omit the DPoP HTTP proof header with invalid_dpop_proof.
 */
@Slf4j
public class StrictDPoPTokenRequestAuthenticationConverter implements AuthenticationConverter {
  ...
  public Authentication convert(HttpServletRequest request) {
    ...
    log.info(
        "Checking DPoP requirement on token request: grantType={}, hasDPoP={}",
        grantType,
        StringUtils.hasText(dpop));

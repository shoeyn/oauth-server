...
/**
 * Enforces server-determined scopes by binding registered client authorized scopes to the
 * authorization code request, mitigating client-side scope manipulation.
 */
@Slf4j
@RequiredArgsConstructor
public class ClientPreDeterminedScopeAuthorizationRequestConverter
  ...
  public Authentication convert(HttpServletRequest request) {
    ...
    if (StringUtils.hasText(request.getParameter("error"))) {
      ...
      if ((!StringUtils.hasText(redirectUri) || !StringUtils.hasText(state))
          && StringUtils.hasText(requestUri)
          && this.authorizationService != null) {
        OAuth2AuthorizationRequest authReq =
            ParEndpointUtils.resolveAuthRequest(requestUri, this.authorizationService);
        if (authReq != null) {
    ...
    if (authentication instanceof OAuth2AuthorizationCodeRequestAuthenticationToken token) {
      ...
      if (token.getClientId() != null) {
        RegisteredClient client =
            this.registeredClientRepository.findByClientId(token.getClientId());
        if (client != null) {
          ...
          Boolean requirePar =
              client
                  .getClientSettings()
                  .getSetting("settings.client.require-pushed-authorization-requests");
          if (Boolean.TRUE.equals(requirePar)) {
            Object requestUri =
                token.getAdditionalParameters() != null
                    ? token.getAdditionalParameters().get("request_uri")
                    : null;
            if (requestUri == null || !StringUtils.hasText(requestUri.toString())) {
              log.warn(
                  "Strict PAR Enforcement: Rejected direct authorization request for client '{}' missing request_uri.",
                  token.getClientId());
              throw new OAuth2AuthorizationCodeRequestAuthenticationException(
                  new OAuth2Error(
                      ...
                      "Client '"
                          + token.getClientId()
                          + "' strictly requires Pushed Authorization Requests (PAR, RFC 9126). "
                          + "Direct authorization requests without 'request_uri' are rejected.",

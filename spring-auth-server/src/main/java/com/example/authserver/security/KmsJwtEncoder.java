...
public class KmsJwtEncoder implements JwtEncoder {
  ...
  public Jwt encode(JwtEncoderParameters parameters) throws JwtEncodingException {
    try {
      ...
      String activeKeyId =
          (jwsHeader.getKeyId() != null && !jwsHeader.getKeyId().isBlank())
              ? jwsHeader.getKeyId()
              : this.keyId;
      if (activeKeyId != null) {
      ...
      for (Map.Entry<String, Object> entry : jwsHeader.getHeaders().entrySet()) {
        ...
        if ("alg".equalsIgnoreCase(key)
            || "kid".equalsIgnoreCase(key)
            || "typ".equalsIgnoreCase(key)) {
          continue;
        }
      ...
      throw new JwtEncodingException(
          "Strict Fail-Closed: An error occurred while attempting to encode JWT via AWS KMS: "
              + ex.getMessage(),
          ex);
    }
  }

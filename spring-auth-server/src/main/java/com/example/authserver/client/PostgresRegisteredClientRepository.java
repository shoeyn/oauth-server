 ...
 *
 * <p>Architecture & Performance:
 *
 * <ul>
 *   <li><b>Durability:</b> All client definitions and cryptographic public keys are persisted
 *       transactionally in PostgreSQL (tables: {@code oauth2_registered_client}, {@code
 *       oauth2_client_public_key}).
 *   <li><b>Sub-Millisecond Reads:</b> Token issuance and PAR validation resolve from an internal
 *       {@link ConcurrentHashMap} near-cache in ~0.001 ms, avoiding database I/O during
 *       steady-state auth flows.
 *   <li><b>Real-Time Cluster Invalidation:</b> Listens to Redis Pub/Sub ({@code
 *       oauth2as:clients:reload}) to synchronize client mutations across multiple Spring Auth
 *       Server instances instantaneously.
 *   <li><b>Organizational Security:</b> Only this Java repository directly communicates with
 *       PostgreSQL; external management clients interact solely via the authenticated
...
public class PostgresRegisteredClientRepository implements RegisteredClientRepository {
  ...
  /**
   * Reloads the in-memory near-cache from PostgreSQL. Guarantees microsecond lookup times during
   * runtime OAuth 2.1 token issuance.
   */
  public synchronized void reloadNearCacheFromDatabase() {
    try {
      ...
      log.info(
          "Near-cache warmed from PostgreSQL: {} client(s) and {} public key(s) loaded into memory.",
          clientsByClientId.size(),
          publicKeysByClientId.size());
    } catch (Exception e) {
      log.error(
          "Failed to load registered clients from PostgreSQL into near-cache: {}",
          e.getMessage(),
          e);
    }
  }
  ...
  /**
   * Converts a ClientConfigDto and persists it to both oauth2_registered_client and
   * oauth2_client_public_key in PostgreSQL.
   */
  ...
  /**
   * Triggered by Redis reload notifications or external events. Refreshes the in-memory near-cache
   * from PostgreSQL.
   */
  ...
  private RegisteredClient toRegisteredClient(ClientConfigDto dto) {
    RegisteredClient.Builder builder =
        RegisteredClient.withId(dto.clientId())
    ...
    long accessTtlMinutes =
        dto.accessTokenTimeToLiveMinutes() != null ? dto.accessTokenTimeToLiveMinutes() : 15L;
    long refreshTtlDays =
        dto.refreshTokenTimeToLiveDays() != null ? dto.refreshTokenTimeToLiveDays() : 30L;

    builder.clientSettings(
        ClientSettings.builder()
            .requireProofKey(dto.requireProofKey() != null ? dto.requireProofKey() : true)
            .requireAuthorizationConsent(
                dto.requireAuthorizationConsent() != null
                    ? dto.requireAuthorizationConsent()
                    : false)
            .setting(
                "settings.client.require-pushed-authorization-requests",
                dto.requirePushedAuthorizationRequests() != null
                    ? dto.requirePushedAuthorizationRequests()
                    : true)
            .build());

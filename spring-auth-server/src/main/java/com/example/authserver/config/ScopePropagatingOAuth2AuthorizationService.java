 ...
 *
 * <p>Enforces the server-determined scope policy: when a client intentionally omits the {@code
 * scope} parameter (as mandated by this server's policy), the registered client's full authorized
 * scope set is automatically injected into the {@link OAuth2Authorization}, its embedded {@link
 * OAuth2AuthorizationRequest}, and any attached {@link OAuth2AccessToken} before persisting to
 * PostgreSQL.
 *
 * <p>This keeps scope enforcement centralised in one place rather than scattered across token
 * customizers and converters.
 */
@Slf4j
public class ScopePropagatingOAuth2AuthorizationService implements OAuth2AuthorizationService {
  ...
  private OAuth2Authorization propagateScopes(OAuth2Authorization authorization) {
    ...
    if (authorization.getAuthorizedScopes() == null
        || !authorization.getAuthorizedScopes().containsAll(clientScopes)) {
    ...
    if (authReq != null
        && (authReq.getScopes() == null || !authReq.getScopes().containsAll(clientScopes))) {
      builder.attribute(
  ...
  private static JdbcOAuth2AuthorizationService buildDelegate(
      ...
      JsonMapper jsonMapper) {
    JdbcOAuth2AuthorizationService svc =

 ...
 * and {@code state} in a signed JWT (the {@code response} parameter) and redirects to the
 * registered redirect_uri. Falls back to 400 Bad Request if redirect context cannot be established
 * securely.
 */
@Slf4j
...
public class JarmErrorResponseHandler implements AuthenticationFailureHandler {
  ...
  public void onAuthenticationFailure(
    ...
    OAuth2AuthorizationCodeRequestAuthenticationToken token =
        authEx.getAuthorizationCodeRequestAuthentication();
    OAuth2Error error = authEx.getError();

    String clientId =
        token != null && StringUtils.hasText(token.getClientId())
            ? token.getClientId()
            : request.getParameter("client_id");
    String redirectUri =
        token != null && StringUtils.hasText(token.getRedirectUri())
            ? token.getRedirectUri()
            : request.getParameter("redirect_uri");
    String state =
        token != null && StringUtils.hasText(token.getState())
            ? token.getState()
    ...
    if (StringUtils.hasText(requestUri)
      ...
      OAuth2AuthorizationRequest authReq =
          ParEndpointUtils.resolveAuthRequest(requestUri, authorizationService);
      if (authReq != null) {
    ...
    if (!client.getRedirectUris().contains(redirectUri)) {
      log.warn(
          "JARM error: redirect_uri '{}' not registered for client '{}'", redirectUri, clientId);
      response.sendError(
          HttpServletResponse.SC_BAD_REQUEST, "Invalid redirect_uri: not registered for client");
      return;
    }
    ...
    if (StringUtils.hasText(error.getDescription())) {
      claimsBuilder.claim("error_description", error.getDescription());
    }
    if (StringUtils.hasText(error.getUri())) {
      claimsBuilder.claim("error_uri", error.getUri());
    }
    if (StringUtils.hasText(state)) {
    ...
    log.info(
        "RFC 9221 JARM: Signed error response (error='{}') issued to client '{}' at '{}'",
        error.getErrorCode(),
        clientId,
        redirectUri);
